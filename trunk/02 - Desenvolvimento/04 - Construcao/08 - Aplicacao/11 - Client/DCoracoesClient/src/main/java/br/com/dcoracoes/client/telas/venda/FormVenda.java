...
//        btnSalvarPDF.setEnabled(false);
    ...
    private void searchRevendedor() {
        try {
            ...
            List<ViewRevendedor> list = bucarRevendedor(revendedor);


            if (list.size() > 0) {
                popularRevendedor(list.get(0).getRevendedor());
    ...
    public void pushToModel() {
        ...
        if (!jtxtData.getText().isEmpty()) {
            pedido.setDataPedido((Date) jtxtData.getValue());
        } else
    ...
    private void delete() {
        ...
        try {
            serverImpl.removePedido(pedido);
            pedido = null;
            ...
            public void tableChanged(TableModelEvent e) {
                ...
                if (e.getColumn() == 0) {
                    ...
                    if (!codigoProdutoCatalogo.isEmpty()) {
                        Produto produto = buscarProduto(codigoProdutoCatalogo);
                        if (produto != null) {
                        ...
                        if (e.getColumn() == 1) {
                            ...
                            try{
                                if(lstItemProduto != null){
                                    ...
                                    if(validExistProdutoCor(refCatalogo, cor)){
                                        tableItemVenda.getModel().setValueAt(null, row, 1);
                                        return;
                                    }
                                }
     ...
     * @param cor 
     */
    private boolean  validExistProdutoCor(String refCatalogo, String cor){
        for (ItemProduto item : lstItemProduto.values()) {
            if(item.getProduto().getReferenciaCatalogo().equals(refCatalogo)
    ...
    private void atualizaCor(String cor, int quantidade, int row) {
        ...
        if (lstProduto != null) {
            if (lstProduto.containsKey(row)) {
                ...
                produto = buscarProduto(produto.getReferenciaCatalogo());
    ...
    private void gerarNumeroOrcamento() {
        try {
            PedidoServerImpl serverImpl = new PedidoServerImpl();
            txtNumeroOrcamento.setText(serverImpl.recCodigoOrcamento());
        } catch (TransException ex) {
            Logger.getLogger(FormVenda.class.getName()).log(Level.SEVERE, null, ex);
            txtNumeroOrcamento.setText("1234");
        }
    }
     ...
     * @return 
     */
    private List<ViewRevendedor> bucarRevendedor(Revendedor revendedor) throws TransException {
    ...
    private void popularDadosPrincipais(Revendedor revendedor) {
        ...
        Date dateNasc = ((PessoaFisica) revendedor.getPessoa()).getDataNascimento();
...
//        tableItemVenda.getDefaultEditor(Integer.class).stopCellEditing();
        ...
        if (pedido.getListaItensPedido() == null) {
            pedido.setListaItensPedido(new ArrayList<ItemPedido>());
        }
        ...
        for (int i = 0; i < x; i++) {
            ...
            itemPedido.setCodigoFiscal(item.getProduto().getCodigoForFiscal());
     ...
     */
    private Produto buscarProduto(String codigoProdutoCatalogo) {
        ...
        try {
            ...
            List<Produto> list = serverImpl.recProdutos(produto, false);

            if (list.isEmpty()) {
                produto = null;
                JOptionPane.showMessageDialog(this, MessageVenda.CODIGO_CATALOGO_INVALIDO, MensagensUtil.ATENCAO, 1);
            } else {
                produto = list.get(0);
            }
            ...
            Logger.getLogger(FormVenda.class.getName()).log(Level.SEVERE, null, ex);
            JOptionPane.showMessageDialog(this, MessageVenda.ERRO_CONSULTAR_PRODUTO, MensagensUtil.ERRO, 0);
        }
...
//        rm.addEditorForRow(row,ed);
     ...
     * @param pedido
     */
    private boolean mantemPedido(PedidoVenda pedido) throws TransException {
        PedidoServerImpl serverImpl = new PedidoServerImpl();
        try {
            pedido = serverImpl.salvarPedidoVenda(pedido);
        } catch (TransException ex) {
            throw ex;
        }
    ...
    /**
     * Faz a validação do credito do cliente e do valor dá compra 
     * e confirma se cliente pode efetuar a compra
     */
