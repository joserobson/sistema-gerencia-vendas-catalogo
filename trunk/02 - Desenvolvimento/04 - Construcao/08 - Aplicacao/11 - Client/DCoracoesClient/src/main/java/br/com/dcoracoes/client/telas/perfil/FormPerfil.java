...
import br.com.dcoracoes.server.model.canalacesso.AssociaPerfilPermissaoFK;
...
public class FormPerfil extends javax.swing.JFrame implements InterfaceCadastroCompleto {
     ...
     * @return 
     */
    private boolean mantemPerfil(Perfil perfil) throws TransException {
        PerfilServerImpl serverImpl = new PerfilServerImpl();
        try {
            perfil = serverImpl.salvarPerfil(perfil, pushToModelPermissoes());
    ...
    /**
     * Recupera todas as permissões em banco e as inseri na grid
     */
    private void buscarTodasPermissoes() {
        try {
            PermissaoServerImpl serveImpl = new PermissaoServerImpl();
            listTodasPermissoes = serveImpl.recTodasPemissoes();
            popularGrid(listTodasPermissoes);
        } catch (TransException ex) {
            Logger.getLogger(FormPerfil.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private List<AssociaPerfilPermissao> pushToModelPermissoes() {
        ...
        for (int i = 0; i < cont; i++) {
            ...
            if (associa == null) {
                ...
                associa.setChaveComposta(new AssociaPerfilPermissaoFK(perfil, listTodasPermissoes.get(i)));
            }
            if ((Boolean) dtm.getValueAt(i, 0)) {
                associa.setStatus(true);
            } else {
                associa.setStatus(false);
            }
